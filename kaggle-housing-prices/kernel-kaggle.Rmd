...
## Table with missing values or numerical values with zero's

The table below provides counts on unknown (a.k.a. missing or numerical zero) values per variable. Please note that missing values are often legitimate. For example when a dwelling does not have a garage no value is provided in the corresponding variables. We will impute them with “None” or a zero further down this analysis.

```{r}
require(funModeling)        ## table with counts on missing values
...
## Define features and response (target)
...
## One-hot encoding and remove factor levels with low frequency
...
## Analyse residuals of ensemble

```{r}
tmp <- train.full.dt
tmp$pred <- predict(greedyEnsemble, newdata = train.full.treat)
